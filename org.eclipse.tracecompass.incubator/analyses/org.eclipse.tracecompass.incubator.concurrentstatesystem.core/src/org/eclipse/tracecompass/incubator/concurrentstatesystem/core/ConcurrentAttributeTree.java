...
/**
 * The concurrent version of AttributeTree class
 *
 * @author fateme faraji daneshgar
 *
 */

public class ConcurrentAttributeTree {
    ...
    private final ConcurrentStateSystem fSs;
     ...
     */
    public ConcurrentAttributeTree(ConcurrentStateSystem ss) {
        fSs = ss;
    ...
    /**
     * "Existing file" constructor. Builds an attribute tree from a "mapping
     * file" or mapping section previously saved somewhere.
     *
     * @param ss
     *            concurrentStateSystem to which this AT is attached
     * @param fis
    ...
    public ConcurrentAttributeTree(ConcurrentStateSystem ss, FileInputStream fis) throws IOException {
        this(ss);
        ...
        for (String[] attrib : attribList) {
            ...
            getQuarkAndAdd("process", ROOT_ATTRIBUTE, curFullAttribute);
     ...
     */
    /*
     * private static String[] decodeFullAttribute(String[] prevPath, String[]
     * curPath) { if (prevPath == null) { return curPath; } String[] diff = new
     * String[curPath.length]; for (int i = 0; i < curPath.length; i++) { if (i
     * < prevPath.length && curPath[i].equals(SERIALIZATION_WILDCARD)) { diff[i]
     * = prevPath[i]; } else { diff[i] = curPath[i]; } } return diff; }
     */
    /**
     ...
     *             If the starting node quark is out of range
     */
    public ProcessQuark getQuarkAndAdd(String processName, int startingNodeQuark, String... subPath) {
        fLock.writeLock().lock();
        int tsPos = 0;
        ProcessQuark pq = new ProcessQuark(0, 0);
        ...
        try {
            ...
            if (knownQuark == INVALID_ATTRIBUTE) {
                /*
                 * The attribute was not in the table previously, and we want to
                 * add it
                 */
                for (String curDirectory : subPath) {
                    ...
                    if (nextNode == null) {
                        ...
                        tsPos = fSs.addEmptyAttribute(processName, fAttributeList.size() - 1);
                    } else if (curDirectory.equals("Processes")) {
                        tsPos = fSs.addEmptyAttribute(processName, 0);
                    }
                    prevNode = nextNode;
                }
                pq.setAttrQuarck(fAttributeList.size() - 1);
                pq.setTsQuarck(tsPos - 1);
                return pq;
            }
            pq.setAttrQuarck(knownQuark);
            pq.setTsQuarck(fSs.getStateSize(processName) + 1);
            return pq;
